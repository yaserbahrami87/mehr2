                ...
                <table class="table table-bordered table-striped text-center" id="table_example">
                    ...
                    @foreach($sliders as $slider)
                        <tr>
                            ...
                            <td>
                                <img src="/images/thumbnail_{{$slider->value}}" width="100px"  />
                            </td>
                            <td>{{$slider->option}}</td>
                            <td>
                                <a href="/admin/setting/sliders_home/{{$slider->id}}/edit" class="btn btn-warning">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                            </td>
                            <td>
                                <form method="post" action="/admin/setting/sliders_home/{{$slider->id}}" onsubmit="return window.confirm('آیا از حذف اسلایدر اطمینان دارید؟')">
                                    {{method_field('DELETE')}}
                                    {{csrf_field()}}
